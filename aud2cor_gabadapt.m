function cr=aud2cor_gab(y,~,rv,sv,gab_filters. alpha)
size(y)
...
for sind=1:length(sv)
    for rind=1:length(rv)
        ...
        cr(sind,rind,:,:)=convolve2(y,gab_filters(sind,rind).G, ...
				    'reflect');
	cr(sind, rind, : , : ) = alpha(sind, rind)*cr(sind, rind, : ,:);
        cr(sind,length(rv)+rind,:,:)=convolve2(y,gab_filters(sind, ...
						  length(rv)+rind).G,'reflect');
        cr(sind,length(rv)+rind,:,:) = alpha(sind, length(rv) + rind)* cr(sind,length(rv)+rind,:,:);
    end
end
